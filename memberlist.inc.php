<?php
	...
	$moveuplink = WB_URL.'/modules/members/move_up.php?';
	$movedownlink = WB_URL.'/modules/members/move_down.php?';
		
	if ($sort_by == 'position DESC') {
	//reverse:	
		$moveuplink = WB_URL.'/modules/members/move_down.php?';
		$movedownlink = WB_URL.'/modules/members/move_up.php?';	
	}
	...
	if ($isalias > 0) $countalias++;
		...
		<tr class="mrow<?php if($isalias > 0) echo " alias";  if($member_id == $hlmember) echo " hilite"; ?>" onmouseover="this.style.backgroundColor = '#F1F8DD'" onmouseout="this.style.backgroundColor = '#ffffff'">
			...
			<td class="membertd2">
				<a href="<?php echo WB_URL; ?>/modules/members/modify_member.php?<?php echo $mod_param.$group_id.$paramdelimiter; ?>member_id=<?php echo $member_id; ?>">
					<?php 
			...
			<?php } ?>
			<td>
				<a href="#" onclick="javascript: confirm_link('<?php echo $TEXT['ARE_YOU_SURE']; ?>', '<?php echo WB_URL; ?>/modules/members/ghost_member.php?<?php echo $mod_param.$group_id.$paramdelimiter; ?>member_id=<?php echo $member_id; ?>');" name="<?php echo $TEXT['DELETE']; ?>">
